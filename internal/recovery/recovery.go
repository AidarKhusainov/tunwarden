...
func Plan(ctx context.Context) PlanResult { return PlanWithOptions(ctx, Options{}) }
...
func Execute(ctx context.Context) ExecuteResult { return ExecuteWithOptions(ctx, Options{}) }
...
func safeText(s string) string { return render.Redact(s) }
...
func commandSucceeded(result CommandResult, err error) bool { return err == nil && result.ExitCode == 0 }
func commandFailedUnexpectedly(result CommandResult, err error) bool { return err != nil || result.ExitCode != 0 }
...
func recovered(candidate Candidate) CleanupResult { return CleanupResult{Candidate: candidate, Status: "recovered"} }
func skipped(candidate Candidate, message string) CleanupResult { return CleanupResult{Candidate: candidate, Status: "skipped", Message: message} }
func failed(candidate Candidate, err error) CleanupResult { return CleanupResult{Candidate: candidate, Status: "failed", Message: err.Error()} }
...
func orderCleanupCandidates(candidates []Candidate) []Candidate {
	...
	sort.SliceStable(ordered, func(i, j int) bool { return weight(ordered[i].Kind) < weight(ordered[j].Kind) })
	return ordered
}
...
func isManagedNFTTarget(family, table string) bool { return strings.TrimSpace(family) == managedNFTFamily && strings.TrimSpace(table) == managedNFTTableName }
...
func sameCleanPath(a, b string) bool { return filepath.Clean(a) == filepath.Clean(b) }
