...
restart_daemon_after_crash() {
  ...
  for _ in $(seq 1 20); do
    if sudo -n systemctl is-active --quiet podlazd.service; then return 0; fi
    sleep 1
  done
...
run_stability_probe() {
  ...
  while true; do
    ...
    check_dns_resolution "stability-${mode}-${iter}"
    if [[ "${mode}" == "tun" ]]; then check_public_egress "stability-${mode}-${iter}"; fi
    sleep 30
  done
...
safe_wrapper_path() {
  local name="$1"
  case "${name}" in
    suspend-resume|network-reconnect|dhcp-renew|dns-change|polkit-gui-auth|polkit-tty-auth) ;;
    *) fail "unsupported host wrapper name: ${name}" ;;
  esac
...
run_host_wrapper_probe() {
  ...
  set +e
  sudo -n env \
    PODLAZ_E2E_DNS_CHECK_HOST="${PODLAZ_E2E_DNS_CHECK_HOST}" \
    PODLAZ_E2E_ACTIVE_MODE="${mode}" \
    timeout "${PODLAZ_E2E_HOST_WRAPPER_TIMEOUT_SECONDS}" "${wrapper}" \
    >"${E2E_ARTIFACT_DIR}/host-wrapper-${name}.stdout" \
    2>"${E2E_ARTIFACT_DIR}/host-wrapper-${name}.stderr"
  local code=$?
  set -e
...
run_host_disruption_probes() {
  [[ "${PODLAZ_E2E_ENABLE_HOST_DISRUPTION}" == "true" ]] || {
    log "host disruption wrappers are disabled; set PODLAZ_E2E_ENABLE_HOST_DISRUPTION=true to run suspend/network/DHCP/DNS/polkit host probes"
    return 0
  }

  local mode="${PODLAZ_E2E_HOST_DISRUPTION_MODE}"
  if [[ -z "${mode}" ]]; then
    if [[ "${PODLAZ_E2E_ENABLE_TUN}" == "true" ]]; then mode="tun"; else mode="proxy-only"; fi
  fi
  case "${mode}" in
    proxy-only) ;;
    tun) [[ "${PODLAZ_E2E_ENABLE_TUN}" == "true" ]] || fail "PODLAZ_E2E_HOST_DISRUPTION_MODE=tun requires PODLAZ_E2E_ENABLE_TUN=true" ;;
    *) fail "unsupported PODLAZ_E2E_HOST_DISRUPTION_MODE=${mode}" ;;
  esac

  local id="$1" available=0 wrapper
  for wrapper in suspend-resume network-reconnect dhcp-renew dns-change polkit-gui-auth polkit-tty-auth; do
    if [[ -x "$(safe_wrapper_path "${wrapper}")" ]]; then
      available=$((available + 1))
    else
      printf '%s\n' "missing: $(safe_wrapper_path "${wrapper}")" >>"${E2E_ARTIFACT_DIR}/host-wrapper-availability.txt"
    fi
  done
  ...
  connect_profile "${mode}" "${id}"
  for wrapper in suspend-resume network-reconnect dhcp-renew dns-change polkit-gui-auth polkit-tty-auth; do
    if [[ -x "$(safe_wrapper_path "${wrapper}")" ]]; then run_host_wrapper_probe "${wrapper}" "${mode}"; fi
  done
...
while IFS= read -r uri; do
  ...
  import_profile_uri "profile-${profile_index}" "${uri}"
done < <(profile_uri_candidates)
...
PODLAZ_COMMIT="${GITHUB_SHA:-e2e-server-coverage}" \
PODLAZ_BUILT="${PODLAZ_E2E_BUILT:-$(date -u '+%b %d %Y')}" \
PODLAZ_DEB_ARCH="${PODLAZ_DEB_ARCH}" \
  bash scripts/build-deb.sh 2>&1 | tee "${E2E_ARTIFACT_DIR}/server-coverage-build-deb.log"
