...
- `appstreamcli` from the AppStream toolchain for MetaInfo validation.
...
The AppStream MetaInfo file is maintained at `packaging/linux/io.github.aidarkhusainov.tunwarden.metainfo.xml` and installed as `/usr/share/metainfo/io.github.aidarkhusainov.tunwarden.metainfo.xml`. It models TunWarden as a CLI-first `console-application`, uses component ID `io.github.aidarkhusainov.tunwarden`, declares metadata license `CC0-1.0`, declares project license `MIT`, provides the `tunwarden` binary, includes homepage and bugtracker URLs, includes OARS content rating metadata, and includes release notes for the current package metadata version.

The MetaInfo release entries are deterministic, reviewable source files. When release/package automation updates packaged versions, the corresponding release entry must be maintained with concise notes and must not include secrets, provider URLs, subscription URLs, generated configs, or raw IDs.
...
- installs shell completion files for bash, zsh, and fish;
